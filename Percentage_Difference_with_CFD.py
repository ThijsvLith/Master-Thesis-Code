...
# fig.savefig(r'C:\TU_Delft\Master\Thesis\Figures overleaf\Results\CFD_comp\Model2_Re1e6_transition_polars.png', dpi=300, bbox_inches='tight')


plt.show()
# ## # --- VALUES FROM KASKPERS MODEL ---
# ic(pd.DataFrame(POSTDATA_final))
# Interpolate POSTDATA_final Cl at alpha = 2 and 8
alpha_query = np.array([2, 8])
interp_Cl_postdata = np.interp(
    alpha_query,                # x locations to interpolate at
    POSTDATA_final[:, 0],       # x from POSTDATA_final
    POSTDATA_final[:, 2]        # y from POSTDATA_final (Cl)
)

# Interpolate CFD_results Cl at alpha = 2 and 8
interp_Cl_cfd = np.interp(
    alpha_query,                # x locations to interpolate at
    cfd_results['Alpha'].values,  # x from cfd_results
    cfd_results['Cl'].values      # y from cfd_results (Cl)
)

# Calculate absolute percentage difference
abs_perc_diff = np.abs(interp_Cl_postdata - interp_Cl_cfd) / np.abs(interp_Cl_cfd) * 100
abs_diff = np.abs(interp_Cl_postdata - interp_Cl_cfd)

print(f"Interpolated POSTDATA_final Cl at alpha=2 and 8: {interp_Cl_postdata}")
print(f"Interpolated CFD_results Cl at alpha=2 and 8: {interp_Cl_cfd}")
print(f"Absolute percentage difference at alpha=2 and 8: {abs_perc_diff}")
print(f"Absolute difference at alpha=2 and 8: {abs_diff}")
