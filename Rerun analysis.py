...
# File paths
base = r"C:\TU_Delft\Master\Thesis\Wind tunnel analysis code\Thijs LTT\Test_TvL\V3\V3_zz_0.05c_top_Re_5e5"
file_main = base + r"\unc_all_V3_zz_0.05c_top_Re_5e5.txt"
file_rerun1 = base + r"\Stall_Rerun1\unc_all_V3_zz_0.05c_top_Re_5e5_Stall_Run.txt"
file_rerun2 = base + r"\Stall_Rerun2\unc_all_V3_zz_0.05c_top_Re_5e5_Rerun2.txt"
...
df_main = df_main.iloc[2230:2342, :]
...
for i, (col, name) in enumerate(zip(params, param_names)):
    for df, label, color in zip([df_main, df_rerun1, df_rerun2], ['Main', 'Rerun1', 'Rerun2'], colors):
        ...
        axs[i].plot(data.index, data, label=f'{label} original', alpha=0.7, color=color)
