...
def load_and_preprocess_data(filename, model, CPwu_column, CPwl_column, PARAM, z_min=340, z_max=380, alpha_col=1, tol=0.05):
...
def extract_measurements(data, STRIPS):
    ...
    alpha = data.iloc[0:, 0].to_numpy(dtype=float)  # First column after reduction is alpha
    Cpwu = data.iloc[0:, n_base:n_base + n_cpwu].to_numpy(dtype=float)
    Cpwl = data.iloc[0:, n_base + n_cpwu:n_base + n_cpwu + n_cpwl].to_numpy(dtype=float)
    diffCp = Cpwl - Cpwu
...
def calculate_correction_factor(alpha, diffCp, presspos, alpha_min, alpha_max, method='Fmincon'):
    ...
    for i in rangealpha:
        ...
        Cl_meas = trapz(diffCp[i, :], presspos)
...
def correction_factors_all_cases(casenames, model, STRIPS, alpha_min, alpha_max, Method):
    alphas_list = []
    factors_list = []
    labels = []
...
def correction_factor_single_case(casename, model, STRIPS, alpha_min, alpha_max, Method, provide_plot=True):
    PARAM = {}
    if model == 'Model2':
        filename = os.path.join(model, casename, 'unc_all_'+casename+'.txt')
    elif model == 'V3':
...
def correction_factors_no_plot(casenames, model, STRIPS, alpha_min, alpha_max, Method,):
    results = {}
    for casename in casenames:
        PARAM = {}
        if model == 'Model2':
